    ...
    public static void main(String[] args) {
        try {
            final CommandLine cmd = CommandLine
                    ...
                    .withHeader("Calculate POS evaluation for TextPro")
            ...
            List<String> guessesTmp = Files.readLines(guessed, Charsets.UTF_8);
            List<String> trueLabels = Files.readLines(gold, Charsets.UTF_8);

            for (int i = 0; i < guessesTmp.size(); i++) {
                String guess = guessesTmp.get(i);
                if (!guess.startsWith("#")) {
                    guesses.add(guess);
                }
            }
