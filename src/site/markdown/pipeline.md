...
Given a text resource, PIKES performs the following NLP tasks:
  ...
  * **Tokenization**, **Part-of-Speech tagging** (POS tagging) and **Word Sense Disambiguation**, to split the text into tokens, each of them annotated with its POS (e.g., proper noun, ...) and disambiguated WordNet synset.
  * **Named Entity Recognition and Classification** and **Entity Linking**, to identify spans of text denoting entities of predefined categories (e.g., persons, organizations, ...) and link them to well-known entities in external resources such as DBpedia.
  * **Temporal Expression Recognition and Normalization**, to identify dates, times and temporal durations in a text. A normalized time value (e.g., the date components), based on the TimeML standard, is extracted. Relative temporal expressions such as ‘yesterday’ are normalized based on the *document creation time*.
  ...
  * **Semantic Role Labeling** (SRL), to annotate occurrences of frames in the text, identifying predicate, arguments and their roles. PIKES consider the SRL annotation with respect to PropBank and NomBank frames. Mapping between different frame systems are available and indeed we exploit them in this work.

ks:InstanceMentions are extracted from NLP annotations according to the following rules:

  * a `ks:TimeMention` is created for each temporal expression; storing the normalized time (represented using the OWL Time ontology) using property `ks:normalizedValue`.
...
Properties `nif:beginIndex`, `nif:endIndex` and `nif:anchorOf` (the mention textual extent) from the NLP Interchange Format (NIF) vocabulary are extracted for `ks:InstanceMentions`, whose URIs are minted according to the NIF offset-based URI scheme. DBpedia URIs produced by entity linking are associated to corresponding `ks:InstanceMentions` via property `ks:linkedTo`.
...
The first goal is accomplished by materializing all the inferences computed with RDFS rules, OWL 2 RL rules for owl:sameAs knowledge propagation, and a custom rule establishing that whenever an instance participates to a frame,
also the instances it ks:includes participate to the frame:
