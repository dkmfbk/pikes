...
//	private EventCorefWordnetSimServer eventCorefWordnetSimServer;
	...
	public void loadModels() throws Exception {
		...
		stanfordProps.setProperty("tokenize.whitespace", config.getProperty("stanford_token_whitespace", "0").equals("1") ? "true" : "false");
		stanfordProps.setProperty("ssplit.eolonly", config.getProperty("stanford_eolonly", "0").equals("1") ? "true" : "false");
		stanfordProps.setProperty("ssplit.newlineIsSentenceBreak", config.getProperty("stanford_break_on_newline", "always"));
		...
		stanfordProps.setProperty("customAnnotatorClass.anna_pos", "eu.fbk.dkm.pikes.tintop.annotators.AnnaPosAnnotator");
		stanfordProps.setProperty("dcoref.maxdist", "5");

		stanfordProps.setProperty("customAnnotatorClass.simple_pos", "eu.fbk.dkm.pikes.tintop.annotators.SimplePosAnnotator");
...
//		}
		...
		for (int sentIndex = 0; sentIndex < stanfordSentences.size(); sentIndex++) {
			...
			for (NerEntity entity : entities) {
				...
				int endIndex = terms.get(entity.getEndToken()).getWFs().get(terms.get(entity.getEndToken()).getWFs().size() - 1).getOffset() +
						terms.get(entity.getEndToken()).getWFs().get(terms.get(entity.getEndToken()).getWFs().size() - 1).getLength();

				logger.debug("Stanford NER entity: " + entity + "");
				logger.debug(String.format("Stanford NER entity: %s (from %d to %d)", entity.getLabel(), startIndex, endIndex));
		...
		if (enableFactuality) {
			...
			while (iterator.hasNext()) {
				...
				for (int location = 0; location < labeling.numLocations(); location++) {
					factuality.addFactualityPart(labeling.labelAtLocation(location).toString(), labeling.valueAtLocation(location));
				}
			}
...
//				cache.addLinguisticProcessor(new LinguisticProcessor("linkedEntities", "machine-linking"));
		...
		if (enableDBPS && (annotators.contains("linking") || annotators.size() == 0)) {
			...
			try {
				LinguisticProcessor linguisticProcessor = new LinguisticProcessor("linked-entities", "DBpedia Spotlight");
				linguisticProcessor.setBeginTimestamp();
				tags = dbp.tag(text);
				linguisticProcessor.setEndTimestamp();
				NAFdocument.addLinguisticProcessor(linguisticProcessor.getLayer(), linguisticProcessor);
